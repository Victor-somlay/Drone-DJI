...
while running:
    ...
    # Modification ici
    displacement_vector_pid_yaw_tanh = np.tanh((pid[0] * error[0] + pid[1] * ierror[0] + pid[2] * (error[0] - perror[0])) / 400)
    ...
    displacement_vector_pid_fb_tanh = np.tanh((pid[0] * error[1] + pid[1] * ierror[1] + pid[2] * (error[1] - perror[1])) / 200)
    ...
    if not abs(displacement_vector[1]) < 40:
        velocity_fb = int(speed/2 * (2/3*displacement_vector_pid_fb_tanh + 1/3*displacement_vector_pid_fb_pid))
    else:
        velocity_fb = 0
        ierror[1] = 0
        error[1] = 0

    displacement_vector_pid_ud_tanh = np.tanh((pid[0] * error[2] + pid[1] * ierror[2] + pid[2] * (error[2] - perror[2])) / 600)
    displacement_vector_pid_ud_pid = (pid[0] * error[2] + pid[1] * ierror[2] + pid[2] * 2 * (error[2] - perror[2])) / 1500
