...
def drone_velocity(speed,image_shape, x, y, w, h):

    # Middle of face and frame coordinates
    face_middle = (x + w // 2, y + h // 2)
    image_middle = (image_shape[1] // 2, image_shape[0] // 2)
    
    size_ref = 100 # if the displacement is positive, the face is bigger than the cube so the drone will go backward ; 100 if it is for eye comparison
    z_axis_displacement = (size_ref - w, size_ref - h)

    # Displacement of face from middle of frame; yaw is 0; NOTE: using only width of face for z axis displacement
    displacement_vector = np.array([image_middle[0] - face_middle[0], z_axis_displacement[0], image_middle[1] - image_middle[1], 0])

    velocity_lr = lr_move(speed,displacement_vector)
    velocity_fb = fb_move(speed,displacement_vector)
    velocity_ud = ud_move(speed,displacement_vector)
    velocity_yaw = yaw_move(speed,displacement_vector)
...
def drone_velocity2(image_shape, head_barycenter, nose_coord, distance2screen):
    ...
    velocity_lr,velocity_ud = lrud_move(distance2screen, image_middle, head_barycenter)
    velocity_fb = fb_move2(distance2screen)
    velocity_yaw = yaw_move2(distance2screen, head_barycenter, nose_coord)
    
    ### Changer la fonction déplacement horizontal
    velocity_yaw = velocity_lr
    velocity_lr = 0
